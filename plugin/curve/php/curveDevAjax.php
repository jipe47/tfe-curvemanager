<?php
class CurveDevAjax extends AjaxPage
{
	public function construct()
	{
		$this->setAccessName("CurveDev");
		...
		switch($query)
		{
			case "getCurve":
				...
				else
				{
					...
					/*
					Log::add("-----------------------------------------------");
					Log::add("nbr_curve = " . $nbr_curve);
					Log::add("nbr_curve_predicted = " . $nbr_curve_predicted);
					Log::add("nbr_curve_tofetch = " . $nbr_curve_tofetch);
					Log::add("gamemode = " . $gameMode);
					Log::add("position = " . $position);
					Log::add("level = " . $level);
					*/
					$finish = false;
					
					/*echo "getGroupByUser:<br />";
					echo "<pre>";
					print_r($groups);
					echo "</pre>";
					*/
					...
					*/
					...
					//Log::add("No, THIS finish!!");
					
				}
				//Log::add(count($array_curve) . " curves loaded.");
